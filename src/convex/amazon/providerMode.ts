 ...
 * provider's userId, with a clientTag to distinguish between clients.
...
export const getClientDetail = query({
  args: { clientId: v.id("providerClients") },
...
export const createClient = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const user = await ctx.db.get(userId);
    if (user?.role !== "provider") throw new Error("Not a provider account");

    return await ctx.db.insert("providerClients", {
...
export const updateClient = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.clientId, patch as any);
    return { ok: true };
  },
});
...
export const deleteClient = mutation({
  ...
  handler: async (ctx, { clientId }) => {
    ...
    const events = await ctx.db
      ...
      .withIndex("by_user_source", (q) => q.eq("userId", userId).eq("source", "provider"))
      .collect();
...
export const ingestCsv = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const uploadId = await ctx.db.insert("csvUploads", {
      providerUserId: userId,
